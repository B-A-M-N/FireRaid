...
| 10 | Remote Cloudflare deployment behavior (`remote-deployment-smoke`) | **LOCALLY_VERIFIED** (owner-hosted reference smoke) | The owner-hosted `fireraid-production.benevolentjoker.workers.dev` showcase is verified separately from customer production. A valid reference smoke must bind the exact clean HEAD into `X-FireRaid-Build`, confirm the authoritative Worker version and env-scoped migrations, return `/health` 200, and reject a genuine-session submission without verification with `403 verification_required`. The demo path may skip the operator-provided edge-limiter attestation; strict customer production may not. **NOT MEASURED**: sustained production traffic, custom-domain routing, remote cron retention, or universal agent efficacy. The exact-SHA release receipt remains external and untracked by design; run `npm run release:smoke:record` only after the corresponding deployment and operator-observed verification flow. |
