...
| 8 | Autonomous-agent efficacy: PRODUCTION_DEFAULT reduces autonomous signup success vs CONTROL | **NOT YET ESTABLISHED** | No completed matched experiment exists yet (P0-1 renamed the arms; the pre-rename benchmark does not measure the shipped treatment). Run E4 in `docs/EXPERIMENTS.md`. |
| 9 | Any per-architecture vulnerability ranking | **NOT YET ESTABLISHED** | Requires #8 plus matched per-extractor cells |
| 10 | Remote Cloudflare deployment behavior | **NOT YET ESTABLISHED** | User-gated: needs real credentials + internet-facing origin smoke |
