/**
 * FR-INV-002: Deterministic profile engine — AS FROZEN AT v1.
 *
 * This is the PRNG AS FROZEN at v1, moved verbatim from the (live)
 * core/prng.ts at the same freeze that produced profile/v1.ts +
 * catalog-v1.ts. The frozen v1 engine (profile/v1.ts) derives from THIS
 * module, not the live prng.ts, so a future PRNG change (counter width,
 * rejection-sampling policy, domain labeling) can never silently shift what
 * v1 derivations produce — the same freeze guarantee the catalog snapshot
 * gives the strategy/template pools. The live prng.ts may evolve for v2;
 * v1 keeps drawing exactly what it drew at the freeze (the goldens pin it).
 *
