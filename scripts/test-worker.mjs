 ...
 *      and the `test` env (→ generated .dev.vars.test, no Turnstile credentials)
 ...
 * loads for `-e <env>`). P1 (P0-AUDIT-3): test and production-SHAPE tests
 * use bootstrap-generated synthetic vars files (LAB_MODE=true for test;
 * LAB_MODE=false, TURNSTILE_MODE=disabled-test for production-test) — never
 * a developer's local .dev.vars.production. */
...
// P1 (P0-AUDIT-3): test and production-test are HERMETIC — the bootstrap
// WRITES their env-specific vars file from its own synthetic SECRET_ENV
// before spawning wrangler (the write itself happens below, right after
// SECRET_ENV is assembled). Without this, a fresh checkout has no ignored
// .dev.vars.test and wrangler can fall back to plain .dev.vars — which on a
// dev machine carries REAL Turnstile credentials. The file is rewritten each
// run (gitignored), and the contamination check below verifies wrangler
// actually loaded it.
const HERMETIC_DEV_VARS = WRANGLER_ENV === "test" || WRANGLER_ENV === "production-test";
...
/**
 * Secrets the Worker needs, matching the generated .dev.vars.<env> exactly
 * (they are also passed explicitly because explicit is safer than implicit).
 */
/**
...
# every run; wrangler -e ${WRANGLER_ENV} loads THIS file, not .dev.vars).
` +
  ...
  log(`env=${WRANGLER_ENV}: wrote hermetic ${DEV_VARS_FILE} (synthetic secrets)`);
}
...
function checkDevVarsSource() {
  if (HERMETIC_DEV_VARS) {
    // test/production-test: wrangler MUST have loaded the file we just wrote.
    ...
    const expected = `.dev.vars.${WRANGLER_ENV}`;
    if (!new RegExp(`(^|/)${expected.replaceAll(".", "\\.")}$`).test(src)) {
      failWithOutput(
        `env=${WRANGLER_ENV} must load ${expected} (the hermetic synthetic file written ` +
        `by this bootstrap), but wrangler reported: ${src || "(no source reported)"}.`
      );
